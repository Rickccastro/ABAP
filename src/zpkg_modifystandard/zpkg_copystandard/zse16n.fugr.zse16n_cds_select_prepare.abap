FUNCTION ZSE16N_CDS_SELECT_PREPARE.
*"--------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*"  TABLES
*"      IT_PARAMETER_FIELDS STRUCTURE  SE16N_SELTAB OPTIONAL
*"  CHANGING
      ...
      IF dd10bv_line-rollname = space.
        SELECT DISTINCT c~tabname     AS tabname,
           ...
           INNER JOIN tadir AS p
              ON p~pgmid    = 'R3TR'
             AND ( p~object   = 'TABL' OR p~object = 'VIEW' )
...
*...........only if reference can be read, take field
            IF sy-subrc = 0 AND
               ls_dfies-valexi IS INITIAL AND
               ls_dfies-checktable IS INITIAL.
              ld_found = 'X'.
              EXIT.
            ENDIF.
          ENDIF.
...
*.....data element has no usage
      ...
      ENDIF.
      CLEAR fields.
*parameter fields labels "3210575
      data(rv_plabel) = cl_dd_ddl_annotation_service=>get_label_4_parameter( EXPORTING entityname    = CONV #( ld_ddlname  )
                                                                                       parametername = CONV #( dd10bv_line-parametername_raw )
                                                                                       ).
          if rv_plabel is initial.
            fields-fieldtext = dd10bv_line-parametername_raw.
          else.
            fields-fieldtext = rv_plabel.
      endif.
* end "3210575
...
*.check if any parameter values are passed
    IF IT_PARAMETER_FIELDS[] IS INITIAL.
...
*      REPLACE ALL OCCURRENCES OF '#' IN c_string WITH space.
    ...
    ELSE.
      ...
      LOOP AT lt_params INTO ls_params.
        ADD 1 TO ld_tabix.
         TRANSLATE ls_params-param TO UPPER CASE.
        READ TABLE it_parameter_fields WITH KEY field   = ls_params-param.
        IF sy-subrc = 0.
          IF ld_tabix < ld_count.
            CONCATENATE c_string ls_params-param '#=#' '''' it_parameter_fields-low '''' ',#'
            INTO c_string.
*.........last one
          ELSE.
            CONCATENATE c_string ls_params-param '#=#' '''' it_parameter_fields-low '''' ' )'
