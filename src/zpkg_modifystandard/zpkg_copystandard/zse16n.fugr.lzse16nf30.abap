...
*.......Entry can be changed -> save old entry
         ...
         Elseif ld_rc = 2.
*..........lock could not be set due to strange key of the table
*..........allow changes anyway
           MESSAGE i555(kz)
              WITH 'Entry could not be locked but can be changed'.
*.......Entry can be changed -> save old entry
           append <wa_save> to <all_table_save>.
           gt_mod-save = sy-tabix.
           gt_mod-used = true.
...
*..........Entry is locked -> undo changes. Send message only once
           ...
           if ld_once <> true.
              MESSAGE i115(wusl) WITH gt_mod-alv_indx.
              ld_once = true.
           endif.
...
data: tabref_double    type ref to data.
...
data: tabref_new       type ref to data.
...
*.Now we have got a unique key -> prepare change documents
  ...
  ls_se16n_cd_key-sdate = sy-datum.  " note 3281959
  ls_se16n_cd_key-stime = sy-uzeit.  " note 3281959
...
*.change documents for non casting fields
  ...
  create data tabref_new type table of (gd-tab).
  assign tabref_new->* to <all_table_n>.
  ...
  define makro_fill_key_fields.
     ...
     loop at gt_fieldcat_key into wa_fieldcat.
        ASSIGN COMPONENT wa_fieldcat-fieldname
                            OF STRUCTURE &1 TO <F1>.
        IF SY-SUBRC <> 0. continue. ENDIF.
        if wa_fieldcat-datatype = 'CLNT'.
           check: not <f1> is initial.
        endif.
...
*.get obligatory description text
  if <del_table>[] is not initial or
     gt_mod[]      is not initial.
...
*.structure for check on doubles
  create data tabref_double type (gd-tab).
  assign tabref_double->* to <double_wa>.
...
*...............also store new line to be able to compare history
                MAKRO_WRITE_CD LD_POS <TXT_WA> 'N' GD-TXT_TAB.
...
*.........also store new line to be able to compare history
          MAKRO_WRITE_CD LD_POS <TAB_WA> 'N' GD-TAB.
*.........If line has been modified, store old one
          READ TABLE <ALL_TABLE_SAVE> INDEX GT_MOD-SAVE
                           ASSIGNING <WA_SAVE>.
       ...
       when type_ins.
          ...
          if gd-read_clnt = true.
             ...
             perform check_exit using c_event_save
                                      ...
                                      gd-tab
                                changing tabref.
*.........in case of HANA, some tables may not have key fields anymore.
*.........This leads to double lines -> check if line already exists
             if gd-comprflag = 'P'.
                makro_fill_key_fields <tab_wa>.
                select single * from (gd-tab)
                                client specified
                                into <double_wa>
                                where (lt_where).
*...............no duprec, insert
...
*...........INDX only allows upper case
            ...
            if not <all_table_n>[] is initial.
               wa_indx-change_type = 'N'.
               EXPORT <all_table_n> TO DATABASE se16n_cd_data2(1N) FROM wa_indx ID ls_indx-guid.
            endif.
