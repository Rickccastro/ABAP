...
*..get parameter id's, but only once per call of transaction
   if sd_parid <> true.
      ...
      GET PARAMETER ID 'SE16N_CURSOR_TABLE' FIELD gd-cursor_table.
      GET PARAMETER ID 'SE16N_LEADING_ZERO' FIELD gd-lzero.   " note 3231981
      IF sy-subrc NE 0."check if user param SE16N_CURSOR_TABLE is not set and take flags instead
        gd-cursor_table = cl_fin_flags=>get_flag_stat(
                                         iv_appl = 'CA'
                                         iv_sub  = 'Tools'
                                         iv_para = 'SE16N_CURSOR_ON_TABLE' ).
      ENDIF.
      if ld_dummy_line > 0.
...
*..if initial call, set cursor to table (and not to DBCON)
   if ( gd-tab = space or gd-tab_save = space )
     and gd-cursor_table is not initial.
...
*..Only if tab changed
   ...
   clear: gd-view.
   CLEAR: gd-proxy_object, gd-proxy_text.
   clear: gd-formula_name.
   clear: gd-entity, gd-ddlname.
...
*      GOTSTATE            =
   ...
   IF SY-SUBRC = 0.
      ...
      IF gd_tabl_text is initial and sy-langu <> 'E'.
*………….get text in English
      CALL FUNCTION 'DDIF_TABL_GET'
        EXPORTING
          NAME                = gd-tab
...
*.....get proxy object if existing
      gd-proxy_object = ls_dd02v-viewref.
*.....get text for proxy
      IF gd-proxy_object <> space.
        ld_view = gd-proxy_object.
...
*..check for entity and ddl
   ...
   IF LT_DFIES[] IS NOT INITIAL.
     DELETE lt_dfies WHERE DATATYPE = 'NODE'.
   ENDIF.
...
*  .....if new table is entered set cursor to first input line
      if sd_start is not initial or gd-cursor_table is initial.
...
*.....in case of views with explicite texts, FIELDTEXT might be empty
      if lt_dfies-fieldtext is initial.
        if lt_dfies-rollname is initial.
           lr_handler = cl_dd_ddl_handler_factory=>create( ).
           TRY.
           data(ld_ddlsource) = lr_handler->get_ddl_name_4_dd_artefact( exporting ddname =  ld_view ).
           CATCH cx_dd_ddl_exception.
              MESSAGE w345(e2) WITH ld_view RAISING no_ddl_source.
           ENDTRY.
           data(lv_label) = cl_dd_ddl_annotation_service=>get_label_4_element( EXPORTING entityname  = CONV #( ld_ddlsource )
                                                                                         elementname = CONV #( lt_dfies-fieldname )
                                                                                         extend      = 'X' )-value.
          if lv_label is not initial.
             lt_dfies-fieldtext = lv_label.
          else.
...
*.....check if user is not allowed for this column
      ...
      endloop.
      if sy-subrc <> 0.
* check if the datatype is .NODE1for CDS views with no DDL name / CDS entity
        if lt_dfies-datatype NE 'NODE' AND
           lt_dfies-fieldname NE '.NODE1'.
          append gt_selfields.
        endif.
      endif.
...
*..check that TIMS-field does not contain 24:00:00
   if gs_selfields-datatype = 'TIMS' OR
      gs_selfields-datatype = 'TIMN'.
      if gs_selfields-low = '240000'.
         gs_selfields-low = '000000'.
         message i333(wusl).
      elseif gs_selfields-high = '240000'.
         gs_selfields-high = '235959'.
         message i333(wusl).
      else.
      endif.
   endif.
...
CHECK: gd-tab <> space.
DATA: ld_is_cds_view.
CALL METHOD CL_DD_DDL_UTILITIES=>IS_CDS_VIEW
      EXPORTING
          name = gd-tab
      IMPORTING
          is_cds_view = ld_is_cds_view.
IF ld_is_cds_view ne space.
        return.
ENDIF.
...
data: ld_aggregate    like se16n_oj_add_dis-aggregate.
...
*....check if join contains any aggregation, grouping must be activated
       select aggregate from se16n_oj_add_dis into ld_aggregate
                   up to 1 rows
                   where oj_key   = gd-ojkey
                     and prim_tab = gd-tab
                     and aggregate <> space.
...
*.cannot be read
*.this is not necessary anymore with new join-logic!
*  if sy-subrc = 0.
...
*..check if table is allowed or not
   ...
   cl_sal_write_event=>write_edit_in_se16n( id_appl  = sy-tcode
                                            id_table = gd-tab ).
...
*.logic for SE16N_EMERGENCY
  IF gd-emergency = true.
*....check that sy-tcode really is se16n_emergency
    IF sy-tcode <> c_emergency.
      IF i_caller = 'S'.
        MESSAGE a340(wusl).
      ELSE.
        MESSAGE a340(wusl) RAISING no_permission.
      ENDIF.
    ENDIF.
*....now check that emergency user really has change-authority for
*....the given table
    ld_view_name = i_tab.
    CALL FUNCTION 'VIEW_AUTHORITY_CHECK'
      EXPORTING
...
*.if the same GUID has already been created, do an endless loop
  ...
  CLEAR itab_selpa.
  MOVE 'LT_HAVV' TO: itab_selpa-field.
  MOVE 'I'       TO: itab_selpa-sign.
  MOVE 'BT'      TO: itab_selpa-option.
  MOVE 'S'       TO: itab_selpa-type.
  LOOP AT lt_having.
    itab_selpa-low    = lt_having-low.
    itab_selpa-high   = lt_having-field.
    APPEND itab_selpa.
  ENDLOOP.
...
*..read field tab
  ...
  IF lv_f4_result IS INITIAL.
    lv_f4_result = 1.            "standard table
  ENDIF.
...
data: ld_field(40).
  ...
  case ld_field.
    ...
    when 'GD-PROXY_OBJECT'.
       perform tab_pick using 3.
  endcase.
...
*..check that TIMS-field does not contain 24:00:00
   if gs_multi_or-datatype = 'TIMS' OR
      gs_multi_or-datatype = 'TIMN'.
      if gs_multi_or-low = '240000'.
         gs_multi_or-low = '000000'.
         message i333(wusl).
      elseif gs_multi_or-high = '240000'.
         gs_multi_or-high = '235959'.
         message i333(wusl).
      else.
      endif.
   endif.
...
*..save all technical settings as user parameters
   ...
   perform set_param using 'SE16N_CURSOR_TABLE' gd-cursor_table.
   perform set_param using 'SE16N_LEADING_ZERO' gd-lzero.   " note 3231981
   message i107(wusl).
...
*.....last ones
      ...
      if gt_order_by_fields[] is initial and
         gt_group_by_fields[] is initial and
         gt_sum_up_fields[]   is initial.
         sort <all_table>.
         DELETE ADJACENT DUPLICATES FROM <all_table>.
         if i_line_det is initial.
           describe table <all_table> lines gd-count.
           gd-number = gd-count.
         endif.
      endif.
    endif.
...
*...............name of field that needs to be selected
                if ls_addf-double_field is not initial.
                  ls_sel-field  = ls_addf-double_field.
                else.
                  ls_sel-field  = ls_addf-field.
                endif.
                ls_sel-option = ls_addf-field_option.
...
* 1. check if the popup need to be display
data: l_tax_audit type char1,
      l_client_dep type char1.
...
*.if not client dependent, do not bother user with client choose
  if gd-clnt = true or
     ...
     gd-no_clnt_auth = true.
    " clent dependent selection in visible
    clear l_client_dep.
   else.
    " clent dependent selection visible
    l_client_dep = ABAP_true.
   endif.

  if l_tax_audit     is not initial or
     l_client_dep    is not initial or
"     gd-txt_tab      is not initial or
     gd-proxy_object is not initial or
     gd-entity       is not initial or
     gd-ddlname      is not initial or
     gd-fda_on       is not initial or
     gd-tech_view    is not initial or
     gd-add_fields   = true or
     gd-hana_active  = true.

*      if sy-uname = 'YANYA'. " note 3285573 enhancement
         call screen 0122 starting at 10 5. " 3285573 enhancement
         clear ok_code.
*      else.   " 3285573 enhancement
*     call screen 0102 starting at 10 5.
*     clear ok_code.
*      endif.  " 3285573 enhancement

  else.
     message i002(wusl). " no more infoemation to display
  endif.
