...
*    endif.
    clear wa_fieldcat.
    wa_fieldcat-datatype = lt_dfies-datatype.
...
*......fieldtext could be empty
       if lt_dfies-fieldtext is initial.
         if lt_dfies-rollname is initial.
           ...
           ENDTRY.
           data(rv_label) = cl_dd_ddl_annotation_service=>get_label_4_element( EXPORTING entityname  = CONV #( ld_ddlsource  )
                                                                                         elementname = CONV #( lt_dfies-fieldname )
                                                                                         extend      = 'X' )-value.
           if rv_label is initial.
           lt_dfies-fieldtext = wa_fieldcat-fieldname.
           else.
             lt_dfies-fieldtext = rv_label.
           endif.
         else.
...
*......note 2767835, decide about tooltip-text
       if gd_flags-se16n_tooltip_business = true.
...
*...Note 2738926
    if gd_flags-se16n_no_conversion = true.
      if lt_dfies-datatype = 'CHAR'.
        wa_fieldcat-parameter0 = true.
      endif.
    endif.
* ... display leading zero - Note 3231981
    if lt_dfies-datatype = 'NUMC' and gd-lzero = true.
       wa_fieldcat-lzero = true.
    else.
       clear wa_fieldcat-lzero.
    endif.
...
*....Note 2738926
     if gd_flags-se16n_no_conversion = true.
*......also switch conversion off for fields without convexit
       if gd-no_convexit = true and
         wa_fieldcat-convexit = space.
         WA_FIELDCAT-NO_CONVEXT = 'X'.
         MODIFY GT_FIELDCAT FROM WA_FIELDCAT INDEX LD_TABIX.
...
*.look in global buffer if the data ref's are already created
  if gd_flags-se16n_dynamic_table_off = space.
    ld_buf = false.
  else.
  ...
  if gd-formula_name is not initial.
    ld_buf = false.
  endif.
...
*            E_DREF_STRUCTURE            =
         ...
         IF sy-subrc <> 0.
            message e433(wusl) with sy-subrc.
         ENDIF.
       else.
    ...
    if ld_buf <> true or
       ...
       if gd_flags-se16n_dynamic_table_off = space.
          CALL FUNCTION 'SE16N_CREATE_DATA_REFERENCE'
            EXPORTING
*              i_tab                       = gd-tab
              it_fieldcatalog             = gt_fieldcat_key
            IMPORTING
              E_DREF_TABLE                = ldkey
            EXCEPTIONS
              ERROR_IN_STRUCTURE_CREATION = 1
...
*.undo temporary changes to gt_fieldcat
  perform formula_undo_key.
...
*.set dummy variant if needed to also work in refresh mode
  if gd-layout_get = true and
     gs_variant-variant = space.
     gs_variant-variant = c_dummy_vari.
     gs_variant-text    = c_dummy_vari.
...
FORM CREATE_ALV_OBJECTS.
  ...
  DATA l_dynnr TYPE sydynnr VALUE '0200'.
  DATA l_rows_occupied TYPE sysrows VALUE 5.

  IF gd-ext_call EQ abap_true.
    l_rows_occupied = 2.
    l_dynnr = '0220'.
  ENDIF.

*..First create a docking container below screen 0200/0220
  DATA(l_ratio) = ( sy-srows - '1.1' * l_rows_occupied ) * 100 / sy-srows.
  CREATE OBJECT g_custom_container
    EXPORTING
      repid = sy-repid
      dynnr = l_dynnr
      side  = g_custom_container->dock_at_bottom
      ratio = COND i( WHEN l_ratio > 95 THEN 95 ELSE l_ratio ).
