...
*.......temporarily add a field to be recognized in structure creation
        wa_fieldcat-key       = 'A'.
...
FORM formula_undo_key .
  ...
  TYPES: ls_fieldcat LIKE LINE OF gt_fieldcat.

  IF NOT gd-formula_name IS INITIAL.
    MODIFY gt_fieldcat FROM VALUE ls_fieldcat( key = space )
        TRANSPORTING key
         WHERE key = 'A'.
  ENDIF.
