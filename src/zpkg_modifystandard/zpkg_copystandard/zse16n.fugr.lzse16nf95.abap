...
*.call central select routine
  ...
  if gd-number = 0 and gd-edit is initial.
    abort = true.
  endif.
...
FORM SELECT_CENTRAL  TABLES   LT_WHERE
  ...
  DATA: ls_client(3)     type c.
  data: ld_having(1).

  TYPES lt_clients_range LIKE RANGE OF ls_client.
  TYPES ls_clients_range  TYPE LINE OF lt_clients_range.
  DATA: ls_clients type ls_clients_range.
  DATA: lt_clients type lt_clients_range.

  DATA: l_flag_next_line TYPE c.  " read next line note 3262991
...
*.......summarize if currency or quantity
        if gs_fieldcat_grp-datatype = 'CURR' or
          ...
          clear field.                     "note 3342534
          field = |{ gs_table_letter-letter }~{ gs_fieldcat_oj_2-org_field }|. "note 3342534
          append field to t_group_by.     "note 3342534
*.......if not, group it
        else.
...
*.......aggregation has been activated
      if gs_fieldcat_oj_2-aggregate <> space.
        if gs_fieldcat_oj_2-field <> gs_fieldcat_oj_2-org_field.
          field = |{ gs_fieldcat_oj_2-aggregate }( { gs_table_letter-letter }~{ gs_fieldcat_oj_2-org_field } ) as { gs_fieldcat_oj_2-field }|.
        else.
...
*.....summarizing has already been done before
      if gs_fieldcat_grp-datatype = 'CURR' or
        ...
        read table gt_sum_up_fields
            with key field   = gs_fieldcat_oj_2-field.
        if sy-subrc = 0.
          continue.
        ENDIF.
      endif.
...
*.in case lt_where contains client (mandt) "note 3116381
if i_clnt_spez = true.
  LOOP AT gt_selfields INTO gs_selfields.
    IF gs_selfields-client = 'X'
       AND ( gs_selfields-rollname = 'MANDT' or gs_selfields-rollname = 'SYMANDT' )  " note 3262991
       AND gs_selfields-high IS INITIAL.
        lt_clients = VALUE lt_clients_range( ( sign = 'I' option = 'EQ' low = gs_selfields-low ) ).
        ls_client = gs_selfields-low.
        EXIT.
    ELSEIF gs_selfields-high IS NOT INITIAL.
        lt_clients = VALUE lt_clients_range( ( sign = 'I' option = 'BT' low = gs_selfields-low  high = gs_selfields-high ) ).
        ls_client = gs_selfields-low.
        EXIT.
    ENDIF.
  ENDLOOP.
  IF ls_client IS INITIAL and gs_selfields-high IS INITIAL.
    IF GD-PROXY_OBJECT IS INITIAL.
      lt_clients = VALUE lt_clients_range( ( sign = 'I' option = 'BT' low = '000' high = '999') ).
    ELSE.
    ls_client = sy-mandt.
    lt_clients = VALUE lt_clients_range( ( sign = 'I' option = 'EQ' low = sy-mandt ) ).
    ENDIF.
  ENDIF.

 data: lt_client_result type string.
 DATA  result  TYPE string.
 DATA  ls_result TYPE string.
 LOOP AT lt_where INTO ls_where.
  IF ( ls_where CS 'MANDT'
    OR ls_where CS 'RCLNT'
    OR ls_where CS 'CLIENT'
    OR ls_where CS 'MANDANT' )
    OR l_flag_next_line = abap_true.  " note 3262991

    DO.
      TRY.
         result  = segment( val   = ls_where
                            index = sy-index
                            sep   = ' OR ' ).

" note 3262991  --- start ---
*         IF result CS 'EQ'.
*         ls_result = substring_after( val = result sub = 'EQ ' ).
*         ls_result = substring( val = ls_result off = 2 ).
*         ls_clients-sign = 'I'.
*         ls_clients-option = 'EQ'.
*         ls_clients-low = ls_result.
...
*         ENDIF.
         ...
         IF result CS 'EQ'.
           ...
           IF ls_result IS NOT INITIAL.
             ...
             ls_clients-low = ls_result.
             APPEND ls_clients TO lt_clients.
           ELSE. " 'EQ' is at the end of line, value is in the next line
             l_flag_next_line = abap_true.
           ENDIF.
         ELSEIF result IS NOT INITIAL.
            ls_clients-sign = 'I'.
           ls_clients-option = 'EQ'.
           ls_result = result+1.
           ls_clients-low = ls_result.
           APPEND ls_clients TO lt_clients.
           CLEAR l_flag_next_line.
         ENDIF.
" note 3262991  --- end ---
   ...
   DELETE lt_where.
   ENDIF.
   IF SY-TABIX EQ '1' AND ls_where CS ' AND ' AND ls_where NS ' BETWEEN '.
     ls_where = substring_after( val = ls_where sub = 'AND ' ).
     DELETE lt_where.
     INSERT ls_where INTO lt_where index 1.
   ENDIF.
  ENDLOOP.
  IF GD-CDS_STRING CS 'MANDT'.
    data(lv_cds_string1) = substring_before( val = GD-CDS_STRING sub = 'a~MANDT' ).
    data(lv_cds_string2) = substring_after( val = GD-CDS_STRING sub = 'b~MANDT AND ' ).
    CONCATENATE lv_cds_string1 lv_cds_string2 INTO data(lv_cds_string) SEPARATED BY space.
    GD-CDS_STRING = lv_cds_string.
  ENDIF.
  DELETE ADJACENT DUPLICATES FROM lt_clients.
ENDIF.
...
*.....normal select into table....................................
      if i_partial <> true.
        if ld_having = true.
          ...
          IF gd-proxy_object IS NOT INITIAL.
            SELECT (t_field) FROM (gd-cds_string)
            USING CLIENT @ls_client
            WHERE (lt_where)
            group by (t_group_by)
            order by (t_order_by)
            %_HINTS HDB @ld_hint
...
*&      <-- GD_CDS_STRING
*&---------------------------------------------------------------------*
FORM select_cds_string  TABLES    lt_parameter_fields
                        USING     VALUE(tab)
...
*     I_TESTMODE                        = ' '
  ...
  IMPORTING
    E_DONE                            = ld_done
    E_DDLNAME                         = ld_ddlname
  TABLES
    IT_PARAMETER_FIELDS               = lt_parameter_fields
  CHANGING
...
*.error occured, do nothing
  IF sy-subrc <> 0.
    ...
    IF sy-subrc = 4.
      abort = true.
    ENDIF.
    IF gd-entity IS NOT INITIAL.
      CONCATENATE gd-entity ' WITH PRIVILEGED ACCESS' INTO ld_cds_string.
    ELSEIF tab = ld_ddlname.
      CONCATENATE tab ' WITH PRIVILEGED ACCESS' INTO ld_cds_string.
    ELSE.
      ld_cds_string = tab.
    ENDIF.
    IF gd-entity IS NOT INITIAL.
  ...
  ELSE.
    READ TABLE GT_FIELDCAT_TXTTAB INTO DY_FIELDCAT
       WITH KEY DATATYPE = 'LANG'
                KEY      = TRUE.
    IF SY-SUBRC = 0 AND ld_cds_string NS ' ON'.
