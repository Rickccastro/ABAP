...
FORM CALL_DRILLDOWN  USING  value(i_row)
  ...
  data: lt_parameter_fields like SE16N_SELTAB.
...
*........if canceled delete stored navigation
         ...
         check: sy-subrc <> 2.
*........drilldown popup pressed, but variant exists
*........variant content has to be changed on the fly
         IF gd-variant_old <> space.
            PERFORM adapt_current_layout TABLES lt_selfields.
         ENDIF.
       endif.
...
*.determine select string in case of DDL-Source
  perform select_cds_string tables   gt_parameter_fields
                            using    gd-tab
...
*&---------------------------------------------------------------------*
*& Form ADAPT_CURRENT_LAYOUT
*&---------------------------------------------------------------------*
*& text
*&---------------------------------------------------------------------*
*&      --> LT_SELFIELDS
*&---------------------------------------------------------------------*
FORM adapt_current_layout TABLES lt_selfields STRUCTURE se16n_selfields.
  ...
  IMPORTING
    ET_SORT = lt_sort.
*..........now fill all fields from lt_Selfields into fieldcat
  LOOP AT lt_selfields INTO ls_selfields
          WHERE group_by = true.
...
*.if external call, give the chance to manipulate
  IF gd-ext_call = true.
*.check if event was handed over
    READ TABLE gt_callback_events INTO ls_callback_events
         WITH KEY callback_event = c_ext_event_fcat.
    IF sy-subrc = 0.
      PERFORM (ls_callback_events-callback_form)
        IN PROGRAM (ls_callback_events-callback_program)
        IF found
        TABLES lt_fieldcatalog
               lt_selfields.
    ENDIF.
  ENDIF.

*.Now save this temporary layout
