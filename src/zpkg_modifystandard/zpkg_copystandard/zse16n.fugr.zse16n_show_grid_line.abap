FUNCTION ZSE16N_SHOW_GRID_LINE.
*"--------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
*> Special detail display for fields of type STRING
  ld_field = i_column.
  ...
  if ls_dfies-datatype = 'STRG'.
     ...
     assign COMPONENT ld_field of STRUCTURE <wa_table> to <f>.
     SPLIT <f> AT cl_abap_char_utilities=>cr_lf INTO TABLE lt_lines.
...
*......fieldtext could be empty
     ...
     assign component wa_fieldcat-fieldname of structure <wa_table>
                      to <f>.
     IF wa_fieldcat-ref_table NE gd-tab.
       CONCATENATE wa_fieldcat-ref_table '-' wa_fieldcat-fieldname INTO gt_detail-fieldname.
     ELSE.
...
*>.......... convert other timestamps into nice format
        elseif ls_dfies-domname  = 'TZNTSTMPS'      OR
               ls_dfies-domname  = 'TZNTSTMPL'      OR
               ls_dfies-domname  = '/SSB/TIMESTAMP' OR
               ls_dfies-rollname = 'TIMESTAMPL'     OR
               ls_dfies-rollname = 'TIMESTAMP'      OR
               ls_dfies-rollname = 'TZNTSTMPS'      OR
               ls_dfies-rollname = 'RSCONTTIMESTMP' OR
               ls_dfies-rollname = 'RSTIMESTMP'.
            convert TIME STAMP <f> TIME ZONE space into date ld_date time ld_time.
            if sy-subrc >= 8.
              clear: ld_date, ld_time.
            endif.
            write ld_date to gt_detail-low(10).
            write ld_time to gt_detail-low+12(8).
*<.......... convert other timestamps into nice format
