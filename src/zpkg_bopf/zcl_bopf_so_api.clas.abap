...
public section.
  ...
  methods CREATE
    importing
      !IS_HEADER type ZBO_SALES_ORDER_HEADER_CS
      !IT_ITEM type ZBO_SALES_ITEM_HEADER_CTT
    exporting
      !ET_MESSAGE type BAPIRET2_T .
  methods DELETE
    importing
  ...
  methods READ_SINGLE
    importing
      !ID_SOID type INT4
    exporting
      !ES_HEADER type ZBO_SALES_ORDER_HEADER_CS
      !ET_ITEM type ZBO_SALES_ITEM_HEADER_CTT
      !ET_MESSAGE type BAPIRET2_T .
  methods UPDATE
    importing
      !IS_HEADER type ZBO_SALES_ORDER_HEADER_CS
...
method ACTION.
  ...
  DATA lr_s_root     TYPE REF TO ZBO_SALES_ORDER_HEADER_CS.
...
method CONSTRUCTOR.
  ...
  me->mo_svc_mngr = /bobf/cl_tra_serv_mgr_factory=>get_service_manager( ZIF_SALES_ORDER_C=>sc_bo_key ).
  me->mo_bo_conf  = /bobf/cl_frw_factory=>get_configuration( ZIF_SALES_ORDER_C=>sc_bo_key ).
endmethod.
...
method CREATE.
  ...
  TRY.
    ...
    <ls_mod>-node        = ZIF_SALES_ORDER_C=>sc_node-root.
    ...
    LOOP AT it_item INTO ls_item.
      ...
      APPEND INITIAL LINE TO lt_mod ASSIGNING <ls_mod>.
      <ls_mod>-node        = ZIF_SALES_ORDER_C=>sc_node-item.
      <ls_mod>-change_mode = /bobf/if_frw_c=>sc_modify_create.
      <ls_mod>-source_node = ZIF_SALES_ORDER_C=>sc_node-root.
      <ls_mod>-association = ZIF_SALES_ORDER_C=>sc_association-root-item.
...
method DELETE.
  ...
  DATA ls_header_ori TYPE ZBO_SALES_ORDER_HEADER_CS.
  DATA lt_item_ori   TYPE ZBO_SALES_ITEM_HEADER_CTT.
  ...
  TRY.
    ...
    lr_s_root ?= me->get_node_row(
      ...
      iv_node_key  = ZIF_SALES_ORDER_C=>sc_node-root
...
method GET_SOID_KEY.
  ...
  <ls_parameter>-attribute_name = ZIF_SALES_ORDER_C=>sc_query_attribute-root-select_by_element-soid.
  <ls_parameter>-sign           = 'I'.
  ...
  CALL METHOD mo_svc_mngr->query
    EXPORTING
      iv_query_key            = ZIF_SALES_ORDER_C=>sc_query-root-select_by_element
...
method READ_ALL.
  ...
  mo_svc_mngr->query(
    EXPORTING
      iv_query_key = ZIF_SALES_ORDER_C=>sc_query-root-select_all
      "iv_query_key = ZIF_SALES_ORDER_C=>sc_query-root-select_by_element
      iv_fill_data = abap_true
    IMPORTING
      eo_message   = lo_message
  ...
  TRY.
    ...
    CALL METHOD me->get_node_row_by_assoc(
      EXPORTING
        iv_key       = lv_soid_key
        iv_node_key  = ZIF_SALES_ORDER_C=>sc_node-root
        iv_assoc_key = ZIF_SALES_ORDER_C=>sc_association-root-item
      RECEIVING
        rr_data      = lt_data
    ).
